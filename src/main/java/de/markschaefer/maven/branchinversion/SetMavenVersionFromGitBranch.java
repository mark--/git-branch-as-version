...
@Mojo(name = "set", defaultPhase = LifecyclePhase.PRE_CLEAN, threadSafe = false, requiresProject = false, inheritByDefault = false)
public class SetMavenVersionFromGitBranch extends AbstractMojo {
    ...
    boolean branchFromEnvironment;

    @Parameter(name = "branchEnvironmentVariable", defaultValue = "GIT_BRANCH")
    String branchEnvironmentVariable;

    @Parameter(name = "releaseBranchPattern", defaultValue = "^release/.*")
	...
	return branch.matches(releaseBranchPattern);
    }

    /**
     * @return the first capturing group matched by the regex
     *         {@link #versionPattern}, as default the MAJOR.MINOR.MICRO part of the
     *         version, where all three parts are expected to be numbers
     * @throws MojoExecutionException if the version does not match or no capturing
     *                                group was defined
     */
